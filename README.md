# MongoDB Deployment with ssl and authentication Demo for Kubernetes (i.e. running on local workstation)

An example project demonstrating the deployment of a MongoDB Replica Set via Kubernetes (Kubernetes running locally on a workstation). Contains example Kubernetes YAML resource files (in the 'resource' folder) and associated Kubernetes based Bash scripts (in the 'scripts' and 'cert' folders) to configure the environment and deploy a MongoDB Replica Set.
